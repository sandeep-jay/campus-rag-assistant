...
<template>
  <li class="group relative">
    ...
    >
      ...
      <span class="flex-1 truncate" aria-hidden="true">{{ session.title }}</span>
    </button>

    <button
      :aria-label="`Delete: ${session.title}`"
      class="absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all focus:opacity-100"
