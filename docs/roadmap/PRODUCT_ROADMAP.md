...
| Priority | Focus |
|----------|--------|
| **Now** | [Agentic Helpdesk Rebuild](./AGENTIC_HELPDESK_REBUILD.md) — Phase 5 (live campus router) shipped behind `CAMPUS_ROUTER_ENABLED=false`; next is calibrating the floor with trajectory eval before flipping the flag on in demos |
