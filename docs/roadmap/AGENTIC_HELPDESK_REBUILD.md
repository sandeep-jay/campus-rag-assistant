...
- **5.3 Agent evidence + web consent.** Propose-solution turns expose structured sources in `AgentTurn` and persisted chat rows. Live Tavily fallback requires an explicit user consent turn when KB retry is empty; mock web search skips consent for CI/demo.
