...
def _has_prior_turns(chat_history: list) -> bool:
    for human, ai in chat_history:
        if (human or "").strip() or (ai or "").strip():
            return True
    return False
...
def make_retrieve_node(rag_service: RAGService):
    def retrieve(state: RagState) -> dict[str, Any]:
        ...
        if documents and not isinstance(documents[0], Document):
            documents = [
                Document(page_content=getattr(d, "page_content", str(d)), metadata=getattr(d, "metadata", {}))
                for d in documents
            ]
...
def make_format_node(rag_service: RAGService):
    def format_response(state: RagState) -> dict[str, Any]:
        ...
        if research_mode == "web":
            metadata["disclaimer"] = (
                "This answer used public web search results. "
                "Verify information against official institutional sources."
            )
        else:
